...
// #include "../include/Inventory.h" // Removido na rodada anterior por não ser usado diretamente
#include <raylib.h>
#include <raymath.h> 
#include <stdbool.h>
...
#include <stddef.h>  // Para NULL (se necessário, embora não diretamente usado aqui)


// --- Variáveis Globais Externas (declaradas em main.c e acessíveis globalmente) ---
...
void InitGameResources(Player players_arr[], Music mainPlaylist_arr[]) {
    for (int i = 0; i < MAX_PLAYERS; i++) { players_arr[i].width = 80; players_arr[i].height = 80; }
    const char *player0ImagePath = "./assets/images/player2.png"; 
    if (MAX_PLAYERS > 0) {
        players_arr[0].txr = LoadTexture(player0ImagePath); 
        if (players_arr[0].txr.id == 0) TraceLog(LOG_WARNING, "[InitGameResources] Falha textura P0: %s", player0ImagePath); 
    }
    const char *player1ImagePath = "./assets/images/player1.png"; 
    if (MAX_PLAYERS > 1) {
        players_arr[1].txr = LoadTexture(player1ImagePath); 
        if (players_arr[1].txr.id == 0) TraceLog(LOG_WARNING, "[InitGameResources] Falha textura P1: %s", player1ImagePath); 
    }
    const char *musicFiles[MAX_SIZE] = { "./assets/songs/Desmeon_-_My_Sunshine.ogg", "./assets/songs/19 William Tell Overture.ogg", "./assets/songs/last-summer-by-ikson.mp3", "./assets/songs/Floatinurboat - Spirit Of Things.mp3" };
    for (int i = 0; i < MAX_SIZE; i++) {
        mainPlaylist_arr[i] = LoadMusicStream(musicFiles[i]);
        if (mainPlaylist_arr[i].stream.buffer == NULL) TraceLog(LOG_WARNING, "[InitGameResources] Falha musica %d: %s", i + 1, musicFiles[i]);
    }
}

void PrepareNewGameSession(Player players_arr[], int *mapX, int *mapY) {
    TraceLog(LOG_INFO, "Preparando nova sessao de jogo..."); // Corrigido para ASCII
    *mapX = 0; *mapY = 0;
    int currentSectionTotalHeight = gameSectionHeightMultiplier * virtualScreenHeight;
    for (int i = 0; i < MAX_PLAYERS; i++) {
        players_arr[i].width = 80; players_arr[i].height = 80;
        if (i == 0) { players_arr[i].posx = players_arr[i].width + 50; players_arr[i].posy = currentSectionTotalHeight / 2 - players_arr[i].height / 2; } 
        else if (i == 1) { players_arr[i].posx = players_arr[i].width + 150; players_arr[i].posy = currentSectionTotalHeight / 2 - players_arr[i].height / 2; }
    }
}
...
void UpdatePlayingScreen(GameState *currentScreen_ptr, Player players_arr[], Music playlist_arr[], 
    ...
    UpdateCameraCenteredOnPlayers(gameCamera, players_arr, MAX_PLAYERS, (float)virtualScreenWidth, (float)virtualScreenHeight);
    ...
    if (IsKeyPressed(KEY_KP_0) || IsKeyPressed(KEY_ZERO)) { if (*isPlaying_ptr && playlist_arr[*currentMusicIndex_ptr].stream.buffer != NULL) { *volume_ptr = 0.0f; SetMusicVolume(playlist_arr[*currentMusicIndex_ptr], *volume_ptr); } }
    if (IsKeyPressed(KEY_KP_SUBTRACT) || IsKeyPressed(KEY_MINUS)) { if (*isPlaying_ptr && playlist_arr[*currentMusicIndex_ptr].stream.buffer != NULL) { *volume_ptr -= 0.05f; if (*volume_ptr < 0.0f) *volume_ptr = 0.0f; SetMusicVolume(playlist_arr[*currentMusicIndex_ptr], *volume_ptr); } }
    if (IsKeyPressed(KEY_KP_ADD) || IsKeyPressed(KEY_EQUAL)) { if (*isPlaying_ptr && playlist_arr[*currentMusicIndex_ptr].stream.buffer != NULL) { *volume_ptr += 0.05f; if (*volume_ptr > 1.0f) *volume_ptr = 1.0f; SetMusicVolume(playlist_arr[*currentMusicIndex_ptr], *volume_ptr); } }
    ...
    if (MAX_PLAYERS > 0) move_character(&players_arr[0].posx, &players_arr[0].posy, players_arr[0].width, players_arr[0].height, KEY_LEFT, KEY_RIGHT, KEY_UP, KEY_DOWN, KEY_RIGHT_SHIFT);
    if (MAX_PLAYERS > 1) move_character(&players_arr[1].posx, &players_arr[1].posy, players_arr[1].width, players_arr[1].height, KEY_A, KEY_D, KEY_W, KEY_S, KEY_LEFT_SHIFT);
    ...
    if (MAX_PLAYERS > 1) {
        ...
        if (performTransition) {
            *currentMapX_ptr = nextMapX; *currentMapY_ptr = nextMapY;
            int currentSectionTotalWidth = gameSectionWidthMultiplier * virtualScreenWidth;
            int currentSectionTotalHeight = gameSectionHeightMultiplier * virtualScreenHeight;
            for (int i = 0; i < MAX_PLAYERS; i++) {
                int playerExitPosY = players_arr[i].posy; int playerExitPosX = players_arr[i].posx; 
                switch (transitionTo) {
                    case BORDER_RIGHT: players_arr[i].posx = players_arr[i].width / 2; players_arr[i].posy = playerExitPosY; break;
                    case BORDER_LEFT: players_arr[i].posx = currentSectionTotalWidth - players_arr[i].width - (players_arr[i].width / 2); players_arr[i].posy = playerExitPosY; break;
                    case BORDER_TOP: players_arr[i].posx = playerExitPosX; players_arr[i].posy = currentSectionTotalHeight - players_arr[i].height - (players_arr[i].height / 2); break;
                    case BORDER_BOTTOM: players_arr[i].posx = playerExitPosX; players_arr[i].posy = players_arr[i].height / 2; break;
                    default: break;
                }
    ...
    // Pressionar P OU Delete para pausar o jogo
    if (IsKeyPressed(KEY_P) || IsKeyPressed(KEY_ESCAPE)) { 
        *currentScreen_ptr = GAMESTATE_PAUSE; 
        ...
        TraceLog(LOG_INFO, "[Game] Tecla P ou DELETE pressionada. Mudando para GAMESTATE_PAUSE.");
    }
    // Pressionar E para abrir o inventário
...
void DrawPlayingScreen(Player players_arr[], float currentVolume, int currentMusicIndex, int isPlaying, int currentMapX, int currentMapY) {
    ...
    DrawText("Controles: P/Del=Pausa, E=Inventario, Setas/WASD=Mover, Shift=Correr", 10, virtualScreenHeight - 30, 10, RAYWHITE); // Atualizado texto de controles
