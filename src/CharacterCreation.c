    ...
    if (creation_step == final_confirmation_step) { 
        if (IsKeyPressed(KEY_ENTER)) { 
            ...
            if (currentGameMode == GAME_MODE_SINGLE_PLAYER && MAX_PLAYERS_SUPPORTED > 1) {
                ...
                UnloadCharacterAnimations(&players[1]); 
            }
            *mapX = 0; *mapY = 0; 
            ...
            if (playlist && currentMusicIndex >= 0 && currentMusicIndex < MAX_MUSIC_PLAYLIST_SIZE && playlist[currentMusicIndex].stream.buffer != NULL && musicIsPlaying_ptr != NULL) { 
                ...
                PlayMusicStream(playlist[currentMusicIndex]); SetMusicVolume(playlist[currentMusicIndex], currentVolume); *musicIsPlaying_ptr = 1; 
            } else if (musicIsPlaying_ptr != NULL) { *musicIsPlaying_ptr = 0; }
...
void DrawCharacterCreationScreen(Player players[]) {
    ...
    if (creation_step < final_confirmation_step) {
        if (player_idx_drawing < playersToDisplay) {
            ...
            if (creation_step % 2 == 0) { // Etapa de Nome
            ...
            } else { // Etapa de Classe e Raça/Sprite
                int leftColumnX = boxX + 20;
                int rightColumnX = boxX + (boxWidth / 2) + 10; // Início da coluna da direita
                int currentLeftY = boxY + 90;
                int currentRightY = boxY + 90;

                DrawText(TextFormat("Nome: %s", creation_player_names[current_config_player_idx]), leftColumnX, boxY + 60, 20, YELLOW);
                
                // Lado Esquerdo: Seleção de Classe
                ...
                for (int i = 0; i < NUM_AVAILABLE_CLASSES; i++) { 
                    ...
                    DrawText(TextFormat("%s %s", (i == creation_class_selection_idx[current_config_player_idx] && !creation_focus_on_sprite_type) ? "->" : "  ", CLASS_DISPLAY_NAMES[i]), leftColumnX + 20, currentLeftY + i * 25, 20, textColor);
                }

                // Lado Direito: Preview de Atributos
                Player previewPlayer; 
                init_player(&previewPlayer, "", AVAILABLE_CLASSES[creation_class_selection_idx[current_config_player_idx]], AVAILABLE_SPRITE_TYPES_ENUM[creation_sprite_selection_idx[current_config_player_idx]]);
                ...
                DrawText(TextFormat("CAR:%d INT:%d AGI:%d SOR:%d", previewPlayer.carisma, previewPlayer.inteligencia, previewPlayer.agilidade, previewPlayer.sorte), rightColumnX, currentRightY, 12, LIGHTGRAY);
                currentRightY += 12 + 20; // Espaço antes da seleção de raça

                // Lado Direito: Seleção de Raça (abaixo de S.P.E.C.I.A.L.)
                DrawText("Escolha sua Raca:", rightColumnX, currentRightY, 20, creation_focus_on_sprite_type ? YELLOW : LIGHTGRAY); 
                currentRightY += 30;
                for (int i = 0; i < NUM_SPRITE_TYPE_CHOICES; i++) {
                    Color textColor = (i == creation_sprite_selection_idx[current_config_player_idx]) ? ORANGE : WHITE;
                    DrawText(TextFormat("%s %s", (i == creation_sprite_selection_idx[current_config_player_idx] && creation_focus_on_sprite_type) ? "->" : "  ", SPRITE_TYPE_DISPLAY_NAMES[i]), rightColumnX + 20, currentRightY + i * 25, 20, textColor);
                }
                DrawText("SETAS: Navegar | TAB: Mudar Foco | ENTER: Confirmar", boxX + 20, virtualScreenHeight - 60, 10, GRAY);
            }
        }
    } else { // Tela de Confirmação Final
        ...
        for(int i=0; i < playersToDisplay; i++) { 
            DrawText(TextFormat("Jogador %d: %s (%s - %s)", i+1, creation_player_names[i], CLASS_DISPLAY_NAMES[creation_class_selection_idx[i]], SPRITE_TYPE_DISPLAY_NAMES[creation_sprite_selection_idx[i]]), boxX + 20, summaryY, 20, WHITE);
            summaryY += 30; 
        }
