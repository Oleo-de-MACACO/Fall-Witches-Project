...
#include "../include/PauseMenu.h"       // For DrawPauseScreen
#include "../include/Game.h"            // For GameState, Player, Music, MAX_PLAYERS_SUPPORTED etc.
#include <stdio.h>                     
...
// extern GameModeType currentGameMode; // Not needed directly, Menu_GetSaveLoadContextMode() is used
extern int currentActivePlayers;    // Used for new game logic if playersToSaveOrLoad is based on it
...
static void InitializeSaveLoadSlotsMenuButtons() {
    ...
    for (int i = 0; i < MAX_SAVE_SLOTS; i++) {
        saveLoadSlotButtons[i] = (MenuButton){ 
            {(float)SLOT_PADDING_SAVE_LOAD, (float)i * ((float)SLOT_HEIGHT_SAVE_LOAD + (float)SLOT_PADDING_SAVE_LOAD), slotWidth, (float)SLOT_HEIGHT_SAVE_LOAD}, // rect
            "",                         // text (dinâmico)
            DARKGRAY,                   // base_color
            GRAY,                       // hover_color
            DARKGRAY,                   // disabled_color
            WHITE,                      // text_color
            true,                       // is_active
            false,                      // is_hovered
            BUTTON_ACTION_NONE          // action
        };
...
static void InitializeSaveLoadConfirmationButtons() {
    ...
    s_confirmationButtons[1] = (MenuButton){ 
        {boxX + (boxWidth - totalBtnWidth)/2.0f + btnWidth + spacing, buttonY, btnWidth, btnHeight}, "Nao",
        MAROON, ORANGE, GRAY, WHITE, true, false, BUTTON_ACTION_NONE
    };
...
void UpdateSaveLoadMenuScreen(GameState *currentScreen_ptr, Player players[], Music playlist[], int currentMusicIndex, float currentVolume, int *musicIsCurrentlyPlaying_ptr, int *currentMapX_ptr, int *currentMapY_ptr, Vector2 virtualMousePos) {
    ...
    if (currentSubState == 0) { 
        if (wheelMove != 0.0f) { 
            saveLoadScrollOffset += wheelMove * (float)SLOT_HEIGHT_SAVE_LOAD * 2.0f; 
                    ...
                    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                        ...
                        if (isInSaveMode) {
                            if (DoesSaveSlotExist(i, modeForSaveLoad)) { Menu_SetSaveLoadSubState(1); s_confirmationButtonsInitialized = false; } 
                                ...
                                if (isNewGameFlow) { PrepareNewGameSession(players, currentMapX_ptr, currentMapY_ptr, playersToProcess); InitializeCharacterCreation(); if(currentScreen_ptr) *currentScreen_ptr = GAMESTATE_CHARACTER_CREATION; } 
                            ...
                            if (DoesSaveSlotExist(i, modeForSaveLoad)) { int numLoaded; if (LoadGame(players, MAX_PLAYERS_SUPPORTED, i, currentMapX_ptr, currentMapY_ptr, modeForSaveLoad, &numLoaded)) { currentActivePlayers = numLoaded; if(currentScreen_ptr) *currentScreen_ptr = GAMESTATE_PLAYING; if (playlist && currentMusicIndex >=0 && currentMusicIndex < MAX_MUSIC_PLAYLIST_SIZE && playlist[currentMusicIndex].stream.buffer != NULL && musicIsCurrentlyPlaying_ptr) { if(IsMusicStreamPlaying(playlist[currentMusicIndex])) StopMusicStream(playlist[currentMusicIndex]); PlayMusicStream(playlist[currentMusicIndex]); SetMusicVolume(playlist[currentMusicIndex], currentVolume); *musicIsCurrentlyPlaying_ptr = 1; } else if (musicIsCurrentlyPlaying_ptr) { *musicIsCurrentlyPlaying_ptr = 0; } } }
                        }
...
void DrawSaveLoadMenuScreen(Player players[], Music playlist[], int currentMusicIndex, int musicIsPlaying, float musicVolume, int mapX, int mapY) {
    ...
    if (prevScreen == GAMESTATE_PAUSE) {
        // CORRIGIDO: Chamada para DrawPauseScreen com 6 argumentos
