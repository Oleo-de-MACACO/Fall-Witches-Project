...
#include <stddef.h> // Adicionado para NULL
...
static void InitializePauseMenuButtons(void) {
    ...
    pauseMenuButtons[3] = (MenuButton){ {startX_twoButtonsPause, currentY, btnWidth, btnHeight}, "Opcoes", COLOR_BUTTON_ACTIVE, COLOR_BUTTON_HOVER, COLOR_BUTTON_DISABLED, COLOR_BUTTON_TEXT, true, false, BUTTON_ACTION_PAUSE_SETTINGS }; // Corrigido "Opções" para ASCII simples para teste
        ...
        if (pauseMenuButtons[i].is_active && CheckCollisionPointRec(virtualMousePos, pauseMenuButtons[i].rect)) {
            ...
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                switch (pauseMenuButtons[i].action) {
                    ...
                    case BUTTON_ACTION_PAUSE_SETTINGS: TraceLog(LOG_INFO, "[PauseMenu] Opcoes (Placeholder)"); break;
                    case BUTTON_ACTION_GOTO_MAIN_MENU: Menu_RequestMainMenu(currentScreen_ptr, playlist, currentMusicIndex, musicIsCurrentlyPlaying_ptr); break;
                    default: break;
                }
            }
        }
...
void DrawPauseMenuElements(void) {
    ...
    DrawText("PAUSADO", (int)(((float)virtualScreenWidth - (float)MeasureText("PAUSADO", 50)) / 2.0f), (int)((float)virtualScreenHeight/2.0f - 120.0f), 50, GRAY); // Casts para (int)
    for (int i = 0; i < NUM_PAUSE_MENU_BUTTONS; i++) {
        ...
        DrawText(pauseMenuButtons[i].text, (int)(pauseMenuButtons[i].rect.x+(pauseMenuButtons[i].rect.width-(float)tw)/2.0f), (int)(pauseMenuButtons[i].rect.y+(pauseMenuButtons[i].rect.height-20.0f)/2.0f), 20, txtC); // Casts para (int)
    }
}
