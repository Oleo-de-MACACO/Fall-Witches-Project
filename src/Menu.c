...
// #include "../include/CharacterCreation.h" // Removido - não usado diretamente por Menu.c
#include <stdio.h>  
#include <string.h> 
#include "raylib.h"
...
void InitializeMainMenuButtons() {
    ...
    float startX_oneButton = ((float)virtualScreenWidth - buttonWidth) / 2.0f;
    ...
    mainMenuButtons[3] = (MenuButton){{startX_twoButtons, currentY_main, buttonWidth, buttonHeight}, "Opcoes", COLOR_BUTTON_ACTIVE, COLOR_BUTTON_HOVER, COLOR_BUTTON_DISABLED, COLOR_BUTTON_TEXT, true, false, BUTTON_ACTION_SETTINGS }; // "Opções" -> "Opcoes"
...
void UpdateMenuScreen(GameState *currentScreen_ptr, Vector2 virtualMousePos) { /* ... (código igual, já fornecido) ... */ 
        ...
        if (mainMenuButtons[i].is_active && CheckCollisionPointRec(virtualMousePos, mainMenuButtons[i].rect)) {
            ...
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                switch (mainMenuButtons[i].action) {
                    ...
                    case BUTTON_ACTION_QUIT_GAME: g_request_exit = true; TraceLog(LOG_INFO, "[Menu Principal] Sair do Jogo"); break;
                    default: break;
                }
            }
        }
...
void DrawMenuScreen(void) { /* ... (código igual com casts, já fornecido) ... */ 
    if (!mainMenuButtonsInitialized) InitializeMainMenuButtons();
...
void UpdateIntroScreen(GameState *currentScreen_ptr, int *introFrames_ptr) { /* ... (código igual, já fornecido) ... */ 
    (*introFrames_ptr)++;
...
void DrawIntroScreen(void) { /* ... (código igual com casts, já fornecido) ... */ 
    ...
    DrawText("Fall Witches Engine v0.0.8", (int)(((float)virtualScreenWidth - (float)MeasureText("Fall Witches Engine v0.0.8", 20)) / 2.0f), (int)((float)virtualScreenHeight/2.0f + 10.0f), 20, LIGHTGRAY);
...
void InitializePlayerModeMenuButtons() { /* ... (código igual com casts, já fornecido) ... */ 
    float buttonWidth = 250.0f; float buttonHeight = 50.0f; float spacingY = 30.0f;
    float totalHeight = (float)NUM_PLAYER_MODE_MENU_BUTTONS * buttonHeight + ((float)NUM_PLAYER_MODE_MENU_BUTTONS - 1.0f) * spacingY;
    float startY = ((float)virtualScreenHeight - totalHeight) / 2.0f;
    float startX = ((float)virtualScreenWidth - buttonWidth) / 2.0f;
    playerModeMenuButtons[0] = (MenuButton){{startX, startY, buttonWidth, buttonHeight}, "Novo Jogo", MAROON, COLOR_BUTTON_HOVER, COLOR_BUTTON_DISABLED, COLOR_BUTTON_TEXT, true, false, BUTTON_ACTION_NEW_GAME_SETUP_SLOT};
    playerModeMenuButtons[1] = (MenuButton){{startX, startY + buttonHeight + spacingY, buttonWidth, buttonHeight}, "Carregar Jogo", MAROON, COLOR_BUTTON_HOVER, COLOR_BUTTON_DISABLED, COLOR_BUTTON_TEXT, true, false, BUTTON_ACTION_LOAD_GAME};
    playerModeMenuButtonsInitialized = true;
}
void UpdatePlayerModeMenuScreen(GameState *currentScreen_ptr, Music playlist[], int currentMusicIndex, float currentVolume, int *isPlaying_ptr, Vector2 virtualMousePos) { /* ... (código igual, já fornecido) ... */ 
    ...
    if (IsKeyPressed(KEY_ESCAPE)) {
        Menu_RequestMainMenu(currentScreen_ptr, playlist, currentMusicIndex, isPlaying_ptr);
    }
}
void DrawPlayerModeMenuScreen(void) { /* ... (código igual com casts, já fornecido) ... */ 
    if (!playerModeMenuButtonsInitialized) InitializePlayerModeMenuButtons();
    ClearBackground(DARKGRAY);
    DrawText("DOIS JOGADORES", (int)(((float)virtualScreenWidth - (float)MeasureText("DOIS JOGADORES", 40)) / 2.0f), (int)((float)virtualScreenHeight / 4.0f), 40, WHITE);
    for (int i = 0; i < NUM_PLAYER_MODE_MENU_BUTTONS; i++) {
...
void Menu_RequestSaveLoadScreen(GameState *currentScreen_ptr, bool isSaving, GameState fromScreen) { /* ... (código igual, já fornecido) ... */ 
    s_is_in_save_mode = isSaving;
    if (fromScreen == GAMESTATE_PLAYER_MODE_MENU && isSaving) {
...
void Menu_RequestMainMenu(GameState *currentScreen_ptr, Music playlist[], int currentMusicIndex, int *musicIsCurrentlyPlaying_ptr) { /* ... (código igual, já fornecido) ... */ 
