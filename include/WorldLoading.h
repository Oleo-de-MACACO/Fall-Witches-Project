...
#define COLOR_FORAGE_SPAWN (Color){0,   255, 255, 255} // Usará CYAN de raylib.h

typedef enum {
    SPAWN_TYPE_PLAYER, SPAWN_TYPE_ENEMY, SPAWN_TYPE_CHEST, SPAWN_TYPE_FORAGEABLE
} SpawnType;

typedef struct {
    Vector2 position; SpawnType type;
} SpawnPoint;

typedef struct {
    Vector2 position; bool isOpen;
} DoorData;
